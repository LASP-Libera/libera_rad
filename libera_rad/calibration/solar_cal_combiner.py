...
OBSID_TO_FACE_IDENTIFIER: dict[int, DataProductIdentifier] = {
    # Face 1
    384: DataProductIdentifier.cal_solar_cal_face1_combined,
    385: DataProductIdentifier.cal_solar_cal_face1_combined,
    386: DataProductIdentifier.cal_solar_cal_face1_combined,
    387: DataProductIdentifier.cal_solar_cal_face1_combined,
    # Face 2
    388: DataProductIdentifier.cal_solar_cal_face2_combined,
    389: DataProductIdentifier.cal_solar_cal_face2_combined,
    390: DataProductIdentifier.cal_solar_cal_face2_combined,
    391: DataProductIdentifier.cal_solar_cal_face2_combined,
    # Face 3
    392: DataProductIdentifier.cal_solar_cal_face3_combined,
    393: DataProductIdentifier.cal_solar_cal_face3_combined,
    394: DataProductIdentifier.cal_solar_cal_face3_combined,
    395: DataProductIdentifier.cal_solar_cal_face3_combined,
}

#: Face number (1, 2, 3) keyed by DataProductIdentifier.
FACE_IDENTIFIER_TO_FACE_NUM: dict[DataProductIdentifier, int] = {
    DataProductIdentifier.cal_solar_cal_face1_combined: 1,
    DataProductIdentifier.cal_solar_cal_face2_combined: 2,
    DataProductIdentifier.cal_solar_cal_face3_combined: 3,
}
...
def algorithm(manifest_path: Path | S3Path) -> Path | S3Path:
    ...
    all_data, _spice_dir = read_all_input_data(input_manifest)
    ...
    face_identifier = get_solar_cal_face(all_data)  # already called in Step 3
    face_num = FACE_IDENTIFIER_TO_FACE_NUM[face_identifier]
    face_obsid_set = {
        obsid for obsid, info in OBSID_TO_FACE_IDENTIFIER.items()
        if info == face_identifier
    }
...
def get_solar_cal_face(all_data: dict[str, xr.Dataset]) -> DataProductIdentifier:
    ...
    DataProductIdentifier
        One of ``cal_solar_cal_face1_combined``, ``cal_solar_cal_face2_combined``,
        or ``cal_solar_cal_face3_combined``.
...
def build_solar_cal_event_datasets(
    ...
    logger.info(
        ...
        rad_sliced.sizes["PACKET"], rad_sample.sizes["PACKET"],
        rad_sliced.sizes["RAD_SAMPLE_FPE_TIME"], rad_sample.sizes["RAD_SAMPLE_FPE_TIME"],
    )
...
def get_product_definition_for_solar_cal_face(face_identifier: DataProductIdentifier) -> Path:
    ...
    Path
        ...
        :data:`libera_rad.config.cal_solar_cal_product_definitions`.
    ...
    """
    product_definition = cal_solar_cal_product_definitions.get(face_identifier)
    if product_definition is None:
        raise ValueError(
            f"No product definition found for solar-cal face: {face_identifier}. "
            f"Available faces: {list(cal_solar_cal_product_definitions.keys())}."
        )
