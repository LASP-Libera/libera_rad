"""Shortwave (SWC) calibration event family merge."""
...
def build_event_dataset(all_data: dict[str, xr.Dataset], event_spec: CalEventSpec) -> xr.Dataset:
    """Merge SWC L1A inputs into one event-windowed dataset.
    ...
    event_spec : CalEventSpec
        ObsID-specific shortwave calibration event specification.

    Returns
    -------
    xr.Dataset
        Merged shortwave calibration event dataset.
    """
    logger.info("Creating shortwave calibration event dataset")
    inputs = l1a_cal_event_utils.select_and_slice_event_inputs(all_data, event_spec)
    sw_cal_event = l1a_combine.merge_l1a_decoded_datasets(inputs)
