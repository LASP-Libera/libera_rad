...
from libera_rad.calibration.combiners import l1a_cal_event_utils
...
def detect_solar_cal_events(
    ...
    nom_hk_scan = l1a_cal_event_utils.load_l1a_product(data_dir, "NOM-HK-DECODED", filter_windows=all_windows)
    ...
    for face_num, obsids_for_face in sorted(face_obsids.items()):
        ...
        for obsid in obsids_for_face:
            face_windows.extend(l1a_cal_event_utils.detect_value_runs(hk_obsids, hk_times, obsid, min_packets))
        if not face_windows:
...
def write_event_inputs(
    ...
    rad_path = _write_windowed_product(
        data_dir,
        output_dir,
        product_name="RAD-SAMPLE-DECODED",
    ...
    pev_path = _write_windowed_product(
        data_dir,
        output_dir,
        product_name="PEV-SW-STAT-DECODED",
...
def _write_windowed_product(
    ...
    if not all_files:
        logger.warning("%s: no files found in %s", product_name, data_dir)
        return None

    selected = l1a_cal_event_utils.filter_files_by_time_window(all_files, [(t0, t1)], pad=np.timedelta64(0, "s"))
    if not selected:
        logger.warning("%s (%s): no files overlap window [%s — %s]", product_name, label, t0, t1)
        return None

    logger.info("%s (%s): loading %d / %d file(s) ...", product_name, label, len(selected), len(all_files))
    ds = l1a_cal_event_utils.open_and_sort_l1a_files(selected)

    ds = l1a_cal_event_utils.slice_dataset_to_time_window(ds, t0, t1, secondary_time_dim=secondary_time_dim)

    n_packets = ds.sizes.get("PACKET", 0)
    if n_packets == 0:
        logger.warning("%s (%s): 0 packets after slicing — skipping.", product_name, label)
        return None
    ...
    if secondary_time_dim:
        ...
        logger.info("%s (%s): %d packets retained.", product_name, label, n_packets)
    ...
    logger.info("%s (%s): written → %s", product_name, label, out_path)
    return out_path
