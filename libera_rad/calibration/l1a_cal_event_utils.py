        ...
        if arr.ndim == 1:
            if arr.shape[0] == n_packets:
                ...
                logger.warning(
                    ...
                    var, arr.shape[0], n_packets, n_fpe,
                )
                data[var] = arr
...
def load_l1a_product(
    ...
    if not all_files:
        raise FileNotFoundError(f"No '*{product_token}*.nc' files in {data_dir}")

    if filter_windows is not None:
        files = filter_files_by_time_window(all_files, filter_windows, pad)
        logger.info("%s: %d / %d file(s) selected by window filter.", product_token, len(files), len(all_files))
    else:
        files = all_files
        logger.info("%s: loading all %d file(s).", product_token, len(all_files))

    if not files:
        raise FileNotFoundError(f"No '{product_token}' files overlap the requested windows.")
...
def scan_files_for_field_windows(
    ...
    """
    files = sorted(data_dir.glob(f"*{product_token}*.nc"))
    if not files:
        raise FileNotFoundError(f"No '*{product_token}*.nc' files in {data_dir}")

    logger.info(
        "Scanning %d %s file(s) for %s values %s ...",
        len(files), product_token, field_var, target_values,
    )
...
def slice_dataset_to_time_window(
    ...
    if secondary_time_dim is not None and secondary_time_dim in ds.dims:
        ...
        logger.debug(
            ...
            t0, t1, int(pkt_mask.sum()), int(sec_mask.sum()), secondary_time_dim,
        )
    else:
        logger.debug(
            "slice_dataset_to_time_window [%s — %s]: %d packets.",
            t0, t1, int(pkt_mask.sum()),
        )
