...
def test_sw_cal_algorithm_end_to_end_conforms_to_product_definition(test_l1a_cal_data_path, cal_io_paths, monkeypatch):
    ...
    event_spec = CAL_EVENT_BY_OBSID[256]

    nom_hk_ds = xr.open_dataset(test_l1a_cal_data_path / "short_nom_hk.nc").load()
    nom_hk_ds["ICIE__SW_OBSID_RAD"] = ("PACKET", np.full(nom_hk_ds.sizes["PACKET"], 256, dtype=np.uint16))
    ...
    write_time_aligned_companion(test_l1a_cal_data_path / "short_cal_sample.nc", cal_sample, t0, t1)
    ...
    monkeypatch.setenv(LIBERA_CAL_OBSID_ENV, "256")
