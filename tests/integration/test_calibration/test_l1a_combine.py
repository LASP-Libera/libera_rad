...
def test_sw_cal_l1a_combine(test_l1a_cal_data_path):
    # Load the test data
    ds_axis = xr.open_dataset(test_l1a_cal_data_path / "short_axis_sample.nc")
    ...
    datasets = [ds_axis, ds_cal_sample, ds_rad_sample, ds_nom, ds_pec, ds_pev]
    merged_ds = merge_l1a_decoded_datasets(datasets)

    assert merged_ds is not None
    assert "PACKET" not in merged_ds.dims
    assert merged_ds.sizes["AXIS_SAMPLE_PACKET"] == 10
...
def test_lw_cal_l1a_combine(test_l1a_cal_data_path):
    ...
    datasets = [ds_axis, ds_rad_sample, ds_nom, ds_pev]
    merged_ds = merge_l1a_decoded_datasets(datasets)
