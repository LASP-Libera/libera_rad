...
from libera_rad.radiometer.radiance import calculate_radiance
...
def test_numerical_radiance_to_dave_results(ground_data, calibration_data, channel_name, pwm_data_name,
                                            dave_radiance_name):
    """Test the conversion of radiance is done the same as Dave's IDL code results"""

    # Convert radiance from pwm signal
    calculated_radiance = calculate_radiance(ground_data[pwm_data_name], ground_data["sci_rad_bench_temp_dn"],
                                             channel_name=channel_name, calibration_data=calibration_data)
    # Load Dave's data
    dave_results = ground_data[dave_radiance_name]

    # Compare the results
    pd.testing.assert_series_equal(dave_results, calculated_radiance, check_dtype=False,
...
def test_nw_per_dn_physical_vs_numerical(ground_data, calibration_data, channel_name, pwm_data_name):
    ...
    numerical_nw = calculate_numerical_nanowatts_per_dn(ground_data[pwm_data_name],
    ...
    physical_nw = calculate_physical_nanowatts_per_dn(ground_data[pwm_data_name],
                                                      bench_temp,
                                                      channel_name=channel_name,
