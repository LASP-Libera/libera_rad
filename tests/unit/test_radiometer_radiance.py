...
def test_physical_to_numerical_radiance(calibration_data, channel_name, measurement_pwm, dark_pwm, temp_dn):
    ...
    """
    numerical_nw = calculate_numerical_nanowatts_per_dn(dark_pwm, temp_dn,
    ...
    physical_nw = calculate_physical_nanowatts_per_dn(dark_pwm,
                                                      bench_temp,
                                                      channel_name=channel_name,
                                                      ground_calibration_info=calibration_data)
...
def test_calculate_numerical_nanowatts_per_dn(calibration_data, dark_pwm, channel_name, heat_sink_temp_dn, known_nw_per_dn):
    """
    Test the calculate_nanowatts_per_dn function
    Taken from Dave's calculations
    """

    calculated_nw_per_dn = calculate_numerical_nanowatts_per_dn(dark_pwm, heat_sink_temp_dn,
                                                                channel_name, calibration_data)
    assert np.abs(calculated_nw_per_dn - known_nw_per_dn) < 1e-5


@pytest.mark.parametrize(
    ("temperature_dn", "known_temperature"),
    [
        (500, 85.52676),
        (1000, 63.17709),
        (1905, 34.17058),
...
def test_watts_per_dn_from_physics(calibration_data, active_pwm, channel_name, heat_sink_temp, known_nw_per_dn):
    ...
    """
    physics_nw_per_dn = calculate_physical_nanowatts_per_dn(active_pwm,
                                                            heat_sink_temp,
                                                            channel_name,
                                                            calibration_data)
...
def test_calculate_resistance_from_temp(calibration_data, channel, trace_path, temperature, resistance):
    ...
    resistance_calculated = calculate_resistance_from_temp(temperature,
                                                           channel_calibration,
...
@pytest.mark.parametrize(
    ...
    [
        (ChannelName.TOTAL, DetectorTracePath.HEATER, 35,
         GeometricResistance(
             silicone_resistance=40.18218,
             leg_resistance=60.83921,
             detector_resistance=14047.7738189)
         ),
        (ChannelName.TOTAL, DetectorTracePath.HEATER, 50,
         GeometricResistance(
             silicone_resistance=40.182176,
             leg_resistance=62.2873198,
             detector_resistance=14716.510486)
         ),
        (ChannelName.TOTAL, DetectorTracePath.HEATER, 65,
         GeometricResistance(
             silicone_resistance=40.1821765,
             leg_resistance=63.7354286,
             detector_resistance=15385.247152)
         ),
        (ChannelName.TOTAL, DetectorTracePath.HEATER, 80,
         GeometricResistance(
             silicone_resistance=40.182176,
             leg_resistance=65.1835375,
             detector_resistance=16053.9838197)
         ),
        (ChannelName.TOTAL, DetectorTracePath.THERMISTOR, 35,
         GeometricResistance(
             silicone_resistance=37.979557,
             leg_resistance=62.279273,
             detector_resistance=5899.671209)
         ),
        (ChannelName.TOTAL, DetectorTracePath.THERMISTOR, 50,
         GeometricResistance(
             silicone_resistance=37.979557,
             leg_resistance=63.762208,
             detector_resistance=6180.625824)
         ),
        (ChannelName.TOTAL, DetectorTracePath.THERMISTOR, 65,
         GeometricResistance(
             silicone_resistance=37.979557,
             leg_resistance=65.245142,
             detector_resistance=6461.580441)
         ),
        (ChannelName.TOTAL, DetectorTracePath.THERMISTOR, 80,
         GeometricResistance(
             silicone_resistance=37.979557,
             leg_resistance=66.72807,
             detector_resistance=6742.53505)
         )
    ]
)
def test_calculate_geometric_resistances(calibration_data, channel, trace_path, temperature, known_results):
    ...
    trace_resistance = calculate_resistance_from_temp(temperature,
                                                      channel_calibration,
                                                      trace_path=trace_path)
    heat_sink_resistance = calculate_resistance_from_temp(heat_sink_temperature,
                                                          channel_calibration,
                                                          trace_path=trace_path)

    calculated_results = calculate_geometric_resistance(trace_resistance,
...
def test_calculate_heater_current(calibration_data, channel, temperature,
                                  known_current):
    """
    ...
    geometric_resistances = calculate_geometric_resistance(trace_resistance,
                                                           heat_sink_resistance,
                                                           channel_specific_calibration_data=channel_calibration,
                                                           trace_path=trace_path)
    current = calculate_heater_current(geometric_resistances,
                                       channel_specific_calibration_data=channel_calibration,
...
def test_calculate_heater_max_power(calibration_data, channel, temperature, known_power):
    ...
    board_calibration = calibration_data.boards[BoardName.EMFPE.value]
    heat_sink_temperature = 35
    mean_power = calculate_heater_max_power(temperature,
...
def test_calculate_thermistor_power(calibration_data, channel, temperature, known_power):
    ...
    thermistor_power = calculate_thermistor_power(temperature,
                                                  heat_sink_temperature,
                                                  channel_specific_calibration_data=channel_calibration,
                                                  board_specific_calibration_data=board_calibration)
...
def test_create_emitted_power_interpolation(calibration_data, channel, emitted_power_first, emitted_power_last):
    ...
    emitted_power_range = create_emitted_power_interpolation(temp_range,
                                                             heat_sink_temp,
                                                             channel_specific_calibration_data=channel_calibration)
