...
from libera_rad.calibration.combiners.lw_cal_combiner import (
    get_lw_event_type,
...
def _patch_extract(nom_hk_ds: xr.Dataset):
    ...
    return patch(
        "libera_rad.calibration.combiners.lw_cal_combiner.extract_input_dataset",
...
class TestGetLwEventType:
    ...
    def test_single_obsid_repeated_across_packets_returns_one_identifier(self):
        ...
        assert result == DataProductIdentifier.cal_lw_temp1_combined
    ...
    def test_non_lw_cal_obsids_alongside_valid_one_are_ignored(self):
        ...
        assert result == DataProductIdentifier.cal_lw_temp3_combined
...
class TestGetProductDefinitionForLwCalEvent:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataProductIdentifier.cal_lw_temp2_combined,
            DataProductIdentifier.cal_lw_temp3_combined,
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                DataProductIdentifier.cal_lw_temp1_combined,
                "LW_TEMP1_COMBINED_product_definition.yml",
            ),
            (
                DataProductIdentifier.cal_lw_temp2_combined,
                "LW_TEMP2_COMBINED_product_definition.yml",
            ),
            (
                DataProductIdentifier.cal_lw_temp3_combined,
                "LW_TEMP3_COMBINED_product_definition.yml",
            ),
        ],
    )
    ...
    def test_raises_value_error_for_unknown_identifier(self):
        """Raises ValueError when the identifier is not in cal_lw_product_definitions."""
        with patch(
            "libera_rad.calibration.combiners.lw_cal_combiner.cal_lw_product_definitions",
            {},
        ):
            with pytest.raises(ValueError, match="No longwave calibration event detected in input manifest"):
                get_product_definition_for_lw_cal_event(DataProductIdentifier.cal_lw_temp1_combined)
    ...
    def test_error_message_includes_identifier_value(self):
        ...
        identifier = DataProductIdentifier.cal_lw_temp1_combined
        with patch(
            "libera_rad.calibration.combiners.lw_cal_combiner.cal_lw_product_definitions",
            {},
        ):
    ...
    def test_each_identifier_maps_to_distinct_path(self):
        ...
        paths = [get_product_definition_for_lw_cal_event(identifier) for identifier in cal_lw_product_definitions]
