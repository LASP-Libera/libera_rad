...
- BREAKING: Replace type-specific calibration combiners and `*-COMBINED` products with a single ObsID-dispatched `cal-combine` algorithm (requires `LIBERA_CAL_OBSID` and `libera-utils >= 5.10.3`). Invoke via `libera-rad cal-combine <manifest>` (docker-compose `cal-combine` service included). Outputs are ObsID-specific: `GAIN`, `NOISE`, `SWC-{λ}NM`, `LWC-{310,320,330,300,305}K`, `SOLAR-{ch}-{PRI,SEC,TER}`.
...
- MAINT: Track the renamed `libera_utils` calibration `DataProductIdentifier`s — `cal_lwc_temp{1,2,3}` became `cal_lwc_{310,320,330}k` (plus new `cal_lwc_{300,305}k` for ObsIDs 323/324), and `cal_lunar_cal{1,2}` became `cal_lunar_{south,north}_pole`. All five LWC blackbody temperatures now resolve to the `lwc` family.
- KNOWN ISSUE: The LWC cal-combine test fixture is still named `LIBERA_L1A_NOM-HK-LWC-TEMP1-TRIMMED_...nc`, which is no longer a valid product token, so `LiberaDataProductFilename` rejects it and the `lwc-320` case of `test_cal_combine_real_sample_event` fails. The fixture needs regenerating (or renaming to `NOM-HK-LWC-310K-TRIMMED`) against the new registry.
